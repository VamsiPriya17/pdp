...
function cart() {
  ...
  useEffect(()=>{
    ...
    if(router.query.user){ 
      fetch(`https://pdprepdp.vercel.app/api/user/${router.query.user}/cart`,{
        method: 'GET',
        headers: {
  ...
  const handleRequest = () =>{
    setRequest(1)
    axios.get(`https://pdprepdp.vercel.app//api/user/${router.query.user}/cart?evaluate=true&product_id=${cartItems[0]._id}`).then((response)=>setOffer(response.data))
